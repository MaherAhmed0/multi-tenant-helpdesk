...
async function tenant() {
  const organization = await createOrganization(db, {
    name: "Ticket API tenant",
    slug: `ticket-api-${randomUUID()}`,
  });
...
async function principal(
  owner: Awaited<ReturnType<typeof tenant>>,
  ...
  const user = await createUser(db, {
    ...
    teamId: role === "AGENT" ? owner.teamId : null,
  });
...
const routes = [
  ["post", "/tickets"],
  ["get", "/tickets"],
  ["get", "/tickets/not-an-id"],
  ["post", "/tickets/not-an-id/messages"],
] as const;
...
function reply(
  ticketId: string,
  body: object = { message: "The problem still happens." },
  actor = customer,
) {
  return request(app)
...
function readTicket(ticketId: string) {
  return db
    ...
    .where("customer_id", "=", customer.auth.userId)
    .where("id", "=", ticketId)
...
function storedTickets(actor = customer) {
  return db
    .selectFrom("tickets")
    .selectAll()
    .where("organization_id", "=", actor.auth.organizationId)
    .where("customer_id", "=", actor.auth.userId)
    .orderBy("id")
    .execute();
}
function seedTicket(actor = customer, createdAt = new Date()) {
  return db
    .insertInto("tickets")
    .values({
      organization_id: actor.auth.organizationId,
      customer_id: actor.auth.userId,
...
describe("customer ticket API", () => {
  it("requires authentication on every route and CUSTOMER role for ticket creation", async () => {
    for (const [method, path] of routes) {
      expect(
        (await request(app)[method](path).send({}).expect(401)).body,
      ).toEqual({ error: "Authentication required" });
      for (const staff of [agent, admin]) {
        if (method !== "post" || path !== "/tickets") continue;
        expect(
          (
            await request(app)
              [method](path)
  ...
  it("creates a default ticket and its first public message using only session identity", async () => {
    const result = await create({
      ...
      .query({
        ...
        customerId: foreign.auth.userId,
      })
      .expect(201);
      ...
      messages: [
        {
          id: expect.any(String),
          body: input.message,
          author: { name: "Sara Ahmed", type: "CUSTOMER" },
          createdAt: expect.any(String),
        },
      ],
    });
    const ticket = (await storedTickets()).find(
      (row) => row.id === result.body.id,
    )!;
    expect(ticket.id[14]).toBe("7");
    expect(ticket).toMatchObject({
      organization_id: own.organizationId,
      customer_id: customer.auth.userId,
      subject: input.subject,
      status: "OPEN",
      priority: "NORMAL",
    ...
    expect(messages[0]).toMatchObject({
      author_user_id: customer.auth.userId,
      body: input.message,
    });
  });
  ...
  it("requires valid session-bound CSRF and rejects client state and malformed content", async () => {
    ...
    for (const token of [undefined, "invalid", foreign.csrf]) {
      ...
      expect((await operation.expect(403)).body).toEqual({
    ...
    for (const extra of [
      ...
      { customerId: neighbor.auth.userId },
      { customer_id: neighbor.auth.userId },
      { userId: neighbor.auth.userId },
      { role: "CUSTOMER" },
      { status: "CLOSED" },
      { priority: "URGENT" },
      { assignedTeamId: own.teamId },
      { assigned_agent_id: agent.auth.userId },
      { voided_at: null },
  ...
  it("rolls back the ticket when initial message persistence fails", async () => {
    ...
    vi.spyOn(ticketRepository, "createTicket").mockImplementationOnce(
      async (...args) => {
    ...
    vi.spyOn(messageRepository, "createTicketMessage").mockImplementationOnce(
      (executor, data) => message(executor, { ...data, body: "   " }),
    );
    await expect(
      createCustomerTicket(customer.auth, input),
    ).rejects.toMatchObject({ constraint: "ticket_messages_body_check" });
  ...
  it("does not insert a message if ticket persistence fails", async () => {
    ...
    await expect(
      createCustomerTicket(customer.auth, { ...input, subject: "" }),
    ).rejects.toMatchObject({ constraint: "tickets_subject_check" });
    expect(message).not.toHaveBeenCalled();
  });

  it.each(["customer", "organization"])(
    "uses existing authentication to block a deactivated %s",
    async (target) => {
  ...
  it("paginates only owned non-voided tickets with deterministic newest-first ordering", async () => {
    ...
    await db
      ...
      .set({
        voided_at: new Date(),
        voided_by_user_id: actor.auth.userId,
        void_reason: "CUSTOMER_WITHDRAWN",
      })
      .where("organization_id", "=", own.organizationId)
      .where("id", "=", voided.id)
      .execute();
    ...
    const page = await request(app)
      ...
      .set("Cookie", actor.cookie)
      .expect(200);
    expect(page.body.tickets.map((row: { id: string }) => row.id)).toEqual(
      ordered.slice(0, 2),
    );
    ...
    for (const row of page.body.tickets)
    ...
    const next = await request(app)
      .get("/tickets?page=2&limit=2")
      .set("Cookie", actor.cookie)
      .expect(200);
    expect(next.body.tickets.map((row: { id: string }) => row.id)).toEqual(
      ordered.slice(2),
    );
    ...
    for (const row of foreignList.body.tickets)
      expect(ownIds).not.toContain(row.id);
  });
  ...
  it("returns only the authorized public conversation in chronological order, never internal notes", async () => {
      ...
      .values([
        {
          organization_id: own.organizationId,
          ticket_id: ticket.id,
          author_user_id: agent.auth.userId,
          body: "Staff reply",
        ...
        },
        {
          organization_id: own.organizationId,
          ticket_id: ticket.id,
          author_user_id: admin.auth.userId,
          body: "Administrator reply",
    ...
    expect(detail.body).toEqual({
      ...
      messages: ordered.map((row) => ({
        id: row.id,
        body: row.body,
        author: authors[row.author_user_id],
    ...
    await db
      .updateTable("users")
      .set({ name: "Renamed Support" })
      .where("organization_id", "=", own.organizationId)
      .where("id", "=", agent.auth.userId)
      .execute();
    const refreshed = await request(app)
      .get(`/tickets/${ticket.id}`)
      .set("Cookie", customer.cookie)
      .expect(200);
    const staffMessage = refreshed.body.messages.find(
      (message: { body: string }) => message.body === "Staff reply",
    );
    expect(staffMessage.author).toEqual({
      name: "Renamed Support",
      type: "STAFF",
    });
    expect(Object.keys(staffMessage).sort()).toEqual([
      "author",
      "body",
      "createdAt",
      "id",
  ...
  it("hides other customers, other tenants, voided tickets and unknown IDs behind the same not-found response", async () => {
    ...
    await db
      ...
      .set({
        voided_at: new Date(),
        voided_by_user_id: customer.auth.userId,
        void_reason: "CUSTOMER_WITHDRAWN",
      })
      .where("organization_id", "=", own.organizationId)
      .where("id", "=", voided.id)
  ...
  it.each(["OPEN", "IN_PROGRESS", "RESOLVED"] as const)(
    "adds a public message to %s, reopening only RESOLVED",
    async (status) => {
      const ticket = await seedTicket();
      const before = await db
        ...
        .returningAll()
        .executeTakeFirstOrThrow();
      const result = await reply(ticket.id, {
        message: " \n Still broken. \t ",
      }).expect(201);
  ...
  it("rejects replies to CLOSED tickets before inserting anything", async () => {
    ...
    expect((await reply(ticket.id).expect(409)).body).toEqual({
  ...
  it("hides unowned, cross-tenant, missing and voided reply targets", async () => {
    ...
    for (const id of [
      ...
      expect((await reply(id).expect(404)).body).toEqual({
  ...
  it("requires reply CSRF and validates only the bounded message body and UUID", async () => {
    ...
    for (const body of [
      ...
      { message: 42 },
      { message: "Reply", authorId: agent.auth.userId },
      { message: "Reply", organizationId: other.organizationId },
      { message: "Reply", customerId: neighbor.auth.userId },
      { message: "Reply", role: "AGENT" },
      { message: "Reply", status: "OPEN" },
      { message: "Reply", subject: "Changed" },
    ])
  ...
  it.each(["OPEN", "RESOLVED"] as const)(
    "rolls back %s message and ticket state if persistence fails",
    async (status) => {
      const ticket = await seedTicket();
      const before = await db
        .updateTable("tickets")
        .set({ status, updated_at: new Date("2020-01-01T00:00:00Z") })
        .where("organization_id", "=", own.organizationId)
        .where("id", "=", ticket.id)
        .returningAll()
        .executeTakeFirstOrThrow();
      ...
      await expect(
        addCustomerMessage(customer.auth, ticket.id, { message: "Reply" }),
      ).rejects.toThrow("Test message insert failure");
      expect(await readMessages(ticket.id)).toEqual([]);
      expect(await readTicket(ticket.id)).toEqual(before);

      const operation =
        status === "RESOLVED" ? "reopenResolvedTicket" : "touchTicket";
      const update = ticketRepository[operation];
      vi.spyOn(ticketRepository, operation).mockImplementationOnce(
        async (...args) => {
          await update(...args);
      ...
      await expect(
        addCustomerMessage(customer.auth, ticket.id, { message: "Reply" }),
      ).rejects.toThrow("Test ticket update failure");
      expect(await readMessages(ticket.id)).toEqual([]);
      expect(await readTicket(ticket.id)).toEqual(before);
    },
  );

  it.each(["CLOSED", "voided"] as const)(
    "revalidates a concurrent %s change after waiting for the ticket lock",
    async (state) => {
      const ticket = await seedTicket();
      await db
        .updateTable("tickets")
      ...
      try {
        await db.transaction().execute(async (trx) => {
          await trx
            ...
            .where("id", "=", ticket.id)
            .execute();
          pending = addCustomerMessage(customer.auth, ticket.id, {
        ...
        expect(await pending).toMatchObject({
          statusCode: state === "CLOSED" ? 409 : 404,
        });
        expect(await readMessages(ticket.id)).toEqual([]);
        const after = await readTicket(ticket.id);
