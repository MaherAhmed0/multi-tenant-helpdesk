...
async function createPlatformCookie() {
  const admin = await createSystemAdmin(db, {
    email: `overview-${randomUUID()}@example.com`, passwordHash: "test-only-password-hash",
...
describe("SYSTEM_ADMIN platform overview", () => {
  it("counts organization/account states and each tenant role without counting privileged identities", async () => {
    ...
    const baseline = await request(app).get("/system-admin/overview").set("Cookie", cookie).expect(200);
    const before = baseline.body;
    expect(before.organizations.total).toBe(before.organizations.active + before.organizations.deactivated);
    expect(before.tenantUsers.total).toBe(before.tenantUsers.active + before.tenantUsers.deactivated);
    expect(before.tenantUsers.total).toBe(before.tenantUsers.byRole.ORGANIZATION_ADMIN
      + before.tenantUsers.byRole.AGENT + before.tenantUsers.byRole.CUSTOMER);

    // Unique additive fixtures keep the test independent of existing test-database rows.
    const organizations = await db.insertInto("organizations").values([
      { name: "Overview active one", slug: `overview-${randomUUID()}` },
      { name: "Overview active two", slug: `overview-${randomUUID()}` },
      { name: "Overview deactivated", slug: `overview-${randomUUID()}`, deactivated_at: new Date() },
    ]).returning("id").execute();
    await db.insertInto("users").values([
      { organization_id: organizations[0]!.id, role: "ORGANIZATION_ADMIN", name: "Active admin", email: `${randomUUID()}@example.com`, password_hash: "test-only-hash" },
      { organization_id: organizations[1]!.id, role: "ORGANIZATION_ADMIN", name: "Inactive admin", email: `${randomUUID()}@example.com`, password_hash: "test-only-hash", deactivated_at: new Date() },
      { organization_id: organizations[0]!.id, role: "AGENT", name: "Active agent", email: `${randomUUID()}@example.com`, password_hash: "test-only-hash" },
      { organization_id: organizations[1]!.id, role: "AGENT", name: "Inactive agent", email: `${randomUUID()}@example.com`, password_hash: "test-only-hash", deactivated_at: new Date() },
      { organization_id: organizations[1]!.id, role: "CUSTOMER", name: "Customer", email: `${randomUUID()}@example.com`, password_hash: "test-only-hash" },
      // This account is active even though its parent organization is deactivated.
      { organization_id: organizations[2]!.id, role: "CUSTOMER", name: "Customer", email: `${randomUUID()}@example.com`, password_hash: "test-only-hash" },
    ]).execute();
    await createPlatformCookie(); // Neither this principal nor its session contributes to tenant counts.

    const response = await request(app).get("/system-admin/overview").set("Cookie", cookie).expect(200);
